...
choose the install directory, or `MESHH_VERSION=v0.1.2` to install a specific
release.
...
Or download a release archive from GitHub Releases. Replace `0.1.2` with the
version you want to install:

```sh
# macOS arm64 example
curl -L https://github.com/meshhai/meshh-tui/releases/download/v0.1.2/meshh_0.1.2_aarch64-apple-darwin.tar.gz -o meshh.tar.gz
tar -xzf meshh.tar.gz
install -m 0755 meshh_0.1.2_aarch64-apple-darwin/meshh /usr/local/bin/meshh
```
...
The terminal client supports three commands:

- `meshh login` authenticates the terminal with MESHH.
- `meshh tui` opens the route delivery terminal UI.
- `meshh update` checks for and installs newer compatible releases.
...
$ meshh tui
meshh-tui v0.1.2 | live | 3 rows | row 1/3
+------------------------------------------------------------------------------+
